...
- `pnpm db:migrate` · `pnpm db:migrate:generate` · `pnpm db:seed`
  - `db:migrate` routes itself: a database with no migration history (fresh self-host, clean CI) gets the fresh-install runner, which marks the TimescaleDB-era migrations (0001–0019) applied without executing them — the shipped `deploy/postgres` image has no `timescaledb.control`, so they *cannot* run. Anything with existing history goes to drizzle's migrator unchanged. Never point `db:migrate:fresh` at an upgrade-path database: it dedupes by content hash, drizzle by a `created_at` watermark, and four migration files were edited after they were applied.
  - Reference data belongs in migrations: the `billing_tier_limits` ladder is seeded by 0100 (`ON CONFLICT DO NOTHING`, so it is inert on existing databases). `db:seed` adds *demo* data — the demo project, subscribers, paywalls — and no deployment runs it.
