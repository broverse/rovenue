...
**Anchorless rows** — RC `store = promotional`, and manually granted entitlements generally — have no store transaction to verify or key on. These are exactly the rows Adapty imports as transaction-less profiles that the operator must then re-grant by hand. **We import them.** The codebase already has the correct representation: `grantComp` (`apps/api/src/services/subscriptions/grant.ts`) writes a `store = "MANUAL"` purchase whose `storeTransactionId` is a synthetic `comp_<id>`, because `purchases.storeTransactionId` and `originalTransactionId` are both `NOT NULL`. The importer mirrors that representation with one change that matters: **the synthetic id is deterministic, derived from the source row's identity, not random** — a random id would duplicate every anchorless row on a re-run and defeat the whole idempotency design. These rows are counted in their own outcome bucket, are never sent to Phase B (there is nothing to verify), and are visibly marked as unverified.

This is not fabricating store data. The line is: we never invent a value that claims to be a real store transaction identifier for a row the store could recognise; we do use the repo's existing synthetic-id convention for a grant that genuinely has no store transaction.
...
- **Never fabricate a currency.** RC's `price_in_usd` is documented as USD-converted, so mapping it to `priceAmount` with `priceCurrency = "USD"` states a known fact, not an assumption. What is forbidden is the opposite case: an amount of *unknown* denomination (a mapped local-price column with no currency column beside it) may never be stamped with a guessed currency. Whether RC ships a raw local-currency + ISO-4217 pair at all is **unconfirmed** — verify before mapping one. `purchases.priceAmount` and `priceCurrency` are both nullable, so "no trustworthy money on this row" is representable and is the correct outcome.
...
- No changes to `subscribers`, `purchases`, `subscriber_access`, or `revenue_events` **schemas**; the importer writes through existing repository functions. **Confirmed against the live schema:** `purchases.verifiedAt` already exists and is the natural marker for the Phase A / Phase B distinction — no new column is needed. Also confirmed: `purchases.productId` is `NOT NULL` and references `products.id`, so an unresolved product is a structural blocker, not a policy choice; `priceAmount`/`priceCurrency` are both nullable; `ownershipType` exists for `FAMILY_SHARED`; `gracePeriodExpires`, `refundDate`, `cancellationDate` and `autoRenewStatus` all exist and receive RC's `grace_period_end_time`, `refunded_at` and `unsubscribe_detected_at` directly.
...
7. A row whose product is not in the project's catalog is reported as unresolved in its own bucket and never causes an invented product. An anchorless/promotional row is imported as a `MANUAL` purchase with a **deterministic** synthetic id, counted in its own bucket, excluded from Phase B — and importing the same file twice creates it exactly once.
