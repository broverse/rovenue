...
- Prompt-injection posture: generated/imported text reaches ONLY localization VALUES and image URLs (URLs validated http(s)); node ids, types, and structural fields come from the server-side assembly/schema constraints, never verbatim from model/store text. The copilot suite's existing prompt-injection tests extend to the new tools. The http(s) scheme gate (`assertUrlSchemes` in `validate-config.ts`) checks image `url.light`/`url.dark` and button `action.url` only — image `alt` is a deliberate exception: it's a modeled leaf string field (same as any other label), populated from listing/generated text like every other localization value, not a URL.
