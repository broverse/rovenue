...
Each entitlement carries more than its active flag — use these fields to drive
subscription UI (the product, the store, the renewal date):
...
`entitlement(id)` returns `Entitlement | null` (null when the user has no such
entitlement cached); `entitlementsAll()` returns every cached entitlement.
