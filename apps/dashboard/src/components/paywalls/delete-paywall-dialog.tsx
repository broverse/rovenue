...
import { useDeletePaywall } from "../../lib/hooks/useProjectPaywalls";
...
export function DeletePaywallDialog({ projectId, paywall, open, onClose, onDeleted }: Props) {
  return (
    <Dialog.Root
      open={open}
      onOpenChange={(next) => {
    ...
    >
      <Dialog.Portal>
        ...
        >
          {open && paywall && (
            <Body projectId={projectId} paywall={paywall} onClose={onClose} onDeleted={onDeleted} />
          )}
        </Dialog.Popup>
      </Dialog.Portal>
    </Dialog.Root>
...
function Body({
  ...
  const del = useDeletePaywall(projectId);
  ...
  const matches = confirm.trim() === paywall.identifier;

  const onConfirm = async () => {
    if (!matches) return;
    setSubmitError(null);
    try {
      await del.mutateAsync(paywall.id);
      onDeleted?.();
      onClose();
    } catch (err) {
      if (err instanceof ApiError && isPaywallInUse(err)) {
        setSubmitError(
          t(
            "paywalls.delete.errors.inUse",
            "This paywall is still referenced by a placement or a running experiment. Remove those references first.",
          ),
        );
        ...
        setSubmitError(
          t("paywalls.delete.errors.generic", "Could not delete the paywall. Please try again."),
        );
      }
    }
  };
    ...
    <>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <Dialog.Title className="text-[15px] font-semibold leading-5">
              {t("paywalls.delete.title", "Delete paywall")}
            </Dialog.Title>
            <Dialog.Description className="mt-0.5 text-[12px] text-rv-mute-500">
              {t("paywalls.delete.subtitle", "Permanently remove this paywall's remote config.")}
            </Dialog.Description>
          </div>
        </div>
        <button
      ...
      <div className="flex flex-col gap-3 px-5 py-4">
        <div className="rounded-md border border-rv-divider bg-rv-c2 px-3 py-2.5">
          <div className="text-[13px] font-semibold text-foreground">{paywall.name}</div>
          <div className="font-rv-mono text-[11px] text-rv-mute-500">{paywall.identifier}</div>
        </div>

        <div>
          <label htmlFor="confirm-paywall-identifier" className="text-[12px] font-medium text-foreground">
            {t("paywalls.delete.confirmLabel", {
              defaultValue: "Type {{identifier}} to confirm",
              identifier: paywall.identifier,
            })}
          </label>
          <input
            id="confirm-paywall-identifier"
            value={confirm}
      ...
      <footer className="flex items-center justify-end gap-2 border-t border-rv-divider px-5 py-3">
        ...
        >
          {del.isPending
            ? t("paywalls.delete.deleting", "Deleting…")
            : t("paywalls.delete.confirm", "Delete paywall")}
        </Button>
      </footer>
    </>
  );
