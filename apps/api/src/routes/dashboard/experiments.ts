...
experimentsRoute.use("*", requireDashboardAuth);

// ----- POST /dashboard/experiments -----
experimentsRoute.post("/", async (c) => {
...
experimentsRoute.get("/:id", async (c) => {
...
experimentsRoute.patch("/:id", async (c) => {
  const id = c.req.param("id");
  ...
  if (existing.status === ExperimentStatus.DRAFT) {
    const body = updateDraftSchema.parse(raw);
    ...
    const body = updateRunningSchema.parse(raw);
...
experimentsRoute.post("/:id/start", async (c) => {
  const id = c.req.param("id");
  const existing = await prisma.experiment.findUnique({ where: { id } });
  if (!existing) {
    throw new HTTPException(404, { message: "Experiment not found" });
...
experimentsRoute.post("/:id/pause", async (c) => {
  const id = c.req.param("id");
  const existing = await prisma.experiment.findUnique({ where: { id } });
  if (!existing) {
    throw new HTTPException(404, { message: "Experiment not found" });
...
experimentsRoute.post("/:id/resume", async (c) => {
  const id = c.req.param("id");
  const existing = await prisma.experiment.findUnique({ where: { id } });
  if (!existing) {
    throw new HTTPException(404, { message: "Experiment not found" });
...
experimentsRoute.post("/:id/stop", async (c) => {
  const id = c.req.param("id");
  const existing = await prisma.experiment.findUnique({ where: { id } });
  if (!existing) {
    throw new HTTPException(404, { message: "Experiment not found" });
  ...
  const body = stopSchema.parse(await c.req.json().catch(() => ({})));
...
experimentsRoute.get("/:id/results", async (c) => {
  const id = c.req.param("id");
  const experiment = await prisma.experiment.findUnique({ where: { id } });
  if (!experiment) {
    throw new HTTPException(404, { message: "Experiment not found" });
