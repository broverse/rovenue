...
export function queryMetricsTools(ctx: ToolContext) {
  return {
    ...
    "query_metrics_churn": tool({
      description:
        "Compute churn rate over a time window. (not implemented in this release)",
      inputSchema: DateRangeArgs,
      execute: async () => {
        throw new Error(
          "query_metrics_churn is not implemented yet — a dedicated ClickHouse view is required.",
        );
      },
    } as unknown as ReturnType<typeof tool>),
    "query_metrics_conversion": tool({
      description:
        "Compute trial-to-paid conversion rate over a time window. (not implemented in this release)",
      inputSchema: DateRangeArgs,
      execute: async () => {
        throw new Error(
          "query_metrics_conversion is not implemented yet — funnel-conversion ClickHouse view does not exist.",
        );
      },
    } as unknown as ReturnType<typeof tool>),
  };
