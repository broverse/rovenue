...
export function registerAllIntentHandlers(): void {
  ...
  registerIntentHandler(
    "action.subscribers.grantAccess",
    async (ctx, payload) => {
      const { subscriberId, entitlementKey, expiresDate } = payload as {
        subscriberId: string;
      ...
      return drizzle.db.transaction(async (tx) => {
        ...
        // a sentinel purchaseId derived from the subscriber + key.
        const sentinelPurchaseId = `manual:${subscriberId}:${entitlementKey}`;
        ...
        await audit(
          {
            ...
            after: { entitlementKey, expiresDate: expiresDate ?? null },
          },
          tx as Parameters<typeof audit>[1],
        );

        return { subscriberId, entitlementKey, granted: true };
      });
    },
  );
