...
// API_KEY_KIND_MISMATCH were previously undocumented as unreachable: each
// throw site in `middleware/api-key-auth.ts` threw a bare `HTTPException`
// with no `cause`, so all four collapsed through middleware/error.ts's
// status→code mapping into the generic UNAUTHORIZED (401) or FORBIDDEN
// (403). As of 2026-09-06 (Task 5, "wire cause so the five codes actually
// reach clients") every one of those throw sites sets `cause` to its own
// ERROR_CODE, so all four are live on the wire.
...
export const ERROR_CATALOG: Record<keyof typeof ERROR_CODE, ErrorCatalogEntry> = {
  ...
  UNAUTHORIZED: {
    ...
    resolution:
      "This is a generic 401 with no more specific code attached — read `message` for detail. If you expected BEARER_REQUIRED, INVALID_API_KEY_FORMAT or INVALID_API_KEY instead, check that you're hitting `apiKeyAuth`-guarded `/v1` routes; other 401 producers in the codebase have not been given typed causes.",
  },
  FORBIDDEN: {
    ...
    resolution:
      "For SDK callers: switch to a secret key for server-to-server endpoints (credit grants and similar) — never ship a secret key in a client app. For dashboard callers: the signed-in user's project role doesn't include the needed capability; ask a project owner to grant it. If you expected API_KEY_KIND_MISMATCH instead, the rejection came from a route-level guard, not `apiKeyAuth`'s own kind check.",
  },
  NOT_FOUND: {
  ...
  BEARER_REQUIRED: {
    ...
    resolution:
      "Send the header as `Authorization: Bearer <key>` (public `rov_pub_...` or secret `rov_sec_...`). No key was presented at all, or it wasn't in Bearer scheme — nothing to rotate or look up in the dashboard yet.",
  },
  INVALID_API_KEY: {
    ...
    resolution:
      "Rotate the key in the dashboard if it's genuinely invalid, revoked, or expired. If the key looks right, double-check it belongs to the project you think it does — a key for a different project also resolves to nothing here.",
  },
  INVALID_API_KEY_FORMAT: {
    ...
    resolution:
      "Verify the key was copied in full and unmodified from the dashboard — a truncated or hand-edited key, or a credential from a different product, is the usual cause of a format mismatch.",
  },
  API_KEY_KIND_MISMATCH: {
    ...
    resolution:
      "Use a secret key for server-to-server endpoints (never ship one in a client app) or a public key for SDK/client endpoints. Read `message` for which kind this specific route required.",
  },
  ...
  STRIPE_NOT_CONNECTED: {
    ...
    resolution:
      "An operator must connect Stripe for this project (dashboard's Stripe Connect flow) — and, for the funnel routes specifically, complete Stripe's own verification so `charges_enabled` is true — before the billing portal, a funnel can be published with a paywall page, or a funnel-payment checkout can proceed.",
  },
