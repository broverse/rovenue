...
fn persist_and_reload() {
    ...
    assert_eq!(loaded.rovenue_id, "rov_abc");
    assert!(loaded.app_user_id.is_none());
...
fn save_is_upsert_keeps_one_row() {
    ...
    repo.save(&row).unwrap();
    row.app_user_id = Some("user_42".into());
    repo.save(&row).unwrap();
    let count: i64 = store
    ...
    assert_eq!(count, 1);
    let loaded = repo.load().unwrap().unwrap();
    assert_eq!(loaded.app_user_id.as_deref(), Some("user_42"));
}
