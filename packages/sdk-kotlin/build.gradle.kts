plugins {
    ...
    `maven-publish`
    signing
    // API reference generation (ROADMAP §11): `./gradlew dokkaHtml` emits
    // static HTML into build/dokka/html (gitignored, same as every other
    // build/ output from this module).
...
tasks.dokkaHtml.configure {
    ...
    dokkaSourceSets.configureEach {
        // `generated/` is uniffi's raw FFI binding output (regenerated by
        // build-bindings.sh, gitignored) — converters, pointer plumbing, and
        // callback trampolines that exist to satisfy the native bridge, not
        // part of the façade's documented surface. Suppressing it keeps the
        // reference to what app developers actually call.
        perPackageOption {
            matchingRegex.set("dev\\.rovenue\\.sdk\\.generated(\\..*)?")
            suppress.set(true)
        }
    }
}
