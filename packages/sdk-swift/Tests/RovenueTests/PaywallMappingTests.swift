...
final class PaywallMappingTests: XCTestCase {
    ...
    // ------------------------------------------------------------------
    // paywallCloseEnvelope — logPaywallClosed's envelope builder
    // ------------------------------------------------------------------

    func test_paywallCloseEnvelope_buildsExpectedShape() {
        let paywall = mapPaywall(makeCorePaywall(), offering: nil)
        let env = paywallCloseEnvelope(paywall: paywall, eventId: "evt_stable_1", occurredAt: "2026-06-20T10:00:00Z")

        XCTAssertEqual(env?.version, 1)
        XCTAssertEqual(env?.eventId, "evt_stable_1")
        XCTAssertEqual(env?.eventType, "paywall_close")
        XCTAssertEqual(env?.occurredAt, "2026-06-20T10:00:00Z")
        XCTAssertEqual(env?.paywallContext?.paywallId, "pw_1")
        XCTAssertEqual(env?.paywallContext?.placementId, "plc_1")
        XCTAssertEqual(env?.paywallContext?.placementRevision, 3)
    ...
    func test_paywallCloseEnvelope_eventIdIsStableAcrossRepeatedCalls() {
        // Regression guard: the eventId must be generated ONCE by the
        // caller (logPaywallClosed) and passed in — not regenerated by this
        // pure function — so retries of the same logical "closed" event
        // dedupe server-side instead of fanning out into duplicates.
        let paywall = mapPaywall(makeCorePaywall(), offering: nil)
        let first = paywallCloseEnvelope(paywall: paywall, eventId: "evt_fixed", occurredAt: "2026-06-20T10:00:00Z")
        let second = paywallCloseEnvelope(paywall: paywall, eventId: "evt_fixed", occurredAt: "2026-06-20T10:00:01Z")
        XCTAssertEqual(first?.eventId, second?.eventId)
    }

    func test_paywallCloseEnvelope_noPresentedContext_returnsNil() {
        // Best-effort/analytics-only: a paywall with no presented_context
        // (should not happen per the core's contract, but the DTO type is
        // Optional) must not crash logPaywallClosed — it just sends nothing.
        let core = makeCorePaywall(includePresentedContext: false)
        let paywall = mapPaywall(core, offering: nil)
        XCTAssertNil(paywallCloseEnvelope(paywall: paywall, eventId: "evt_x", occurredAt: "2026-06-20T10:00:00Z"))
    }

    func test_paywallCloseEnvelope_omitsOptionalAttributionFieldsWhenAbsent() {
        // A direct (non-experiment) paywall assignment has no variantId/experimentKey.
        let core = makeCorePaywall(variantId: nil, experimentKey: nil)
        let paywall = mapPaywall(core, offering: nil)
        let env = paywallCloseEnvelope(paywall: paywall, eventId: "evt_x", occurredAt: "2026-06-20T10:00:00Z")
        XCTAssertNil(env?.paywallContext?.variantId)
        XCTAssertNil(env?.paywallContext?.experimentKey)
